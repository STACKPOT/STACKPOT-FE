...
const ContractContent: React.FC<ContractContentProps> = ({
  ...
  return contractType === "service" ? (
    <p css={modalContentTextStyle}>
      ...
      <span css={modalContentBoldStyle}>제5조 (서비스 이용 준칙)</span>
      {"\n"}① 회원은 서비스를 본래의 목적과 용도에 맞게 사용하여야 하며, 다음 각
      호의 행위를 하여서는 아니 됩니다. 있습니다.
      {"\n"}1. 법령 또는 공공질서·선량한 풍속에 위반하는 행위
      {"\n"}2. 타인의 권리나 이익을 침해하는 행위
      {"\n"}3. 회사의 서비스 운영을 방해하거나 안정성을 저해하는 행위
      {"\n"}② 회원이 본 조를 위반하여 발생하는 모든 법적 책임은 회원 본인에게
      있으며, 이로 인해 회사에 손해가 발생한 경우 회원은 회사에 그 손해를
      배상하여야 합니다.{"\n"}
    </p>
  ) : (
    <p css={modalContentTextStyle}>
